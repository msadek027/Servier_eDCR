...
namespace eDCR.Areas.DCR.Models.DAL.DAO
{
    public class DoctorDataUploadDAO : ReturnData
    {
        ...
        public List<DoctorDataUploadInfo> GetDoctorGridData(DoctorDataUploadInfo model)
        {
            ...
            string Qry = "Select DOCTOR_ID,DOCTOR_NAME,REG_NO,PHONE,EMAIL,ADDRESS1,ADDRESS2,ADDRESS3,ADDRESS4,RELIGION,DESIGNATION,SPECIALIZATION,POTENTIAL,DEGREES,GENDER,MORNING_LOCATION,EVENING_LOCATION,MARKET_CODE,MARKET,Region, MONTH_NUMBER,YEAR, PRODUCT_GROUP, TEAM_TARGET" +
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                dData.ProductGroup = row["PRODUCT_GROUP"].ToString();
                dData.TeamTarget = row["TEAM_TARGET"].ToString();
        ...
        public List<DoctorDataUploadInfo> GetMarkets()
        {
            string Qry = "SELECT DISTINCT MARKET_CODE, MARKET_NAME FROM HR_LOC_MAPPING WHERE MARKET_CODE IS NOT NULL ORDER BY MARKET_CODE";
            DataTable dt = dbHelper.GetDataTable(dbConn.SAConnStrReader(), Qry);
            ...
            foreach (DataRow row in dt.Rows)
            {
                DoctorDataUploadInfo dData = new DoctorDataUploadInfo();
                dData.MarketCode = row["MARKET_CODE"].ToString();
                dData.Market = row["MARKET_NAME"].ToString();
                item.Add(dData);
            }
            return item;
        }
