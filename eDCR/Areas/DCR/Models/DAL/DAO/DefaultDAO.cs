...
namespace eDCR.Areas.DCR.Models.DAL.DAO
{
    public class DefaultDAO
    {
        ...
        public string GetWhereClauseDefineAsUserLocCode(string LocCode,string EmpType)
        {
            ...
            else if (EmpType == "Manager" || EmpType == "Sr. Manager")
            {
        ...
        public string GetImmediateSubordinate_WhereClauseDefineAsUserLocCode(string LocCode, string EmpType)
        {
            ...
            {
                WhereClause = "  TERRITORY_CODE='" + LocCode + "' AND DESIGNATION IN ('MPO','SMPO')";
            }
            else if (EmpType == "RSM")
            {
                WhereClause = "  REGION_CODE='" + LocCode + "' AND DESIGNATION='TM'";
            }
            else if (EmpType == "DSM")
            {
                WhereClause = "  DIVISION_CODE='" + LocCode + "' AND DESIGNATION='RSM'";
            }
            else if (EmpType == "Executive" || EmpType == "Sr. Executive")
            {
                WhereClause = "  DIVISION_CODE='" + LocCode + "' AND DESIGNATION='DSM'";
            }
            else if (EmpType == "Manager" || EmpType == "Sr. Manager")
            {
                WhereClause = "  M_ID='" + LocCode + "' AND DESIGNATION IN ('RSM','DSM') ";
            }
            else if (EmpType == "EMA")
            {
                WhereClause = "  1=1 ";
        ...
        {
            string Qry = "SELECT  LOC_CODE,MPO_NAME||' - '||MPO_CODE||' , '|| DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE MPO_NAME from VW_HR_LOC_MAPPING_ALL Where DESIGNATION IN ('MPO','SMPO') ";
        ...
        {         
            string Qry = " SELECT  MPO_CODE,MPO_NAME||' - '||MPO_CODE ||' , '||DESIGNATION||' | 'MARKET_NAME||' - '||MARKET_CODE EmpName,LOC_CODE from VW_HR_LOC_MAPPING_ALL Where DESIGNATION IN ('MPO','SMPO','TM','RSM','DSM','Manager')";
            if (TerritoryManagerID != "" && TerritoryManagerID != null)
            {
        ...
        public List<ProductInfo> GetProductItem(ProductInfo model)
        {
            ...
            else
            {
                ...
                else if (HttpContext.Current.Session["Designation"].ToString() == "RSM")
                {
                ...
                else if(HttpContext.Current.Session["Designation"].ToString() == "DSM")
                {
                ...
                else if (HttpContext.Current.Session["Designation"].ToString() == "Manager" || HttpContext.Current.Session["Designation"].ToString() == "Sr. Manager")
                {
          ...
          public List<DefaultBEL> GetEmpForSup()
           {
            ...
            {
                Qry = " Select LOC_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where REGION_CODE ='" + LocCodeSession + "' AND DESIGNATION ='TM'";
            }
            else if (EmpTypeSession == "DSM")
            {
            ...
            {
                Qry = "Select  LOC_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName,DESIGNATION from VW_HR_LOC_MAPPING_ALL Where (DIVISION_CODE ='" + LocCodeSession + "' OR M_ID ='" + LocCodeSession + "') AND LOC_CODE !='"+LocCodeSession+"' AND DESIGNATION  IN ('RSM','DSM','Manager','Sr. Manager')";
        ...
        public List<DefaultBEL> GetEmpCodeForOwnSupfrmDesignation(string Designation)
        {
            ...
            if (EmpTypeSession == "TM")
            {
                if (Designation == "TM")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where TERRITORY_CODE='" + LocCodeSession + "' AND DESIGNATION='" + Designation + "' ";
                }
                if (Designation == "MPO")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where TERRITORY_CODE='" + LocCodeSession + "' AND DESIGNATION IN ('MPO','SMPO') ";
                }
            }
            if (EmpTypeSession == "RSM")
            {
                ...
                if (Designation == "TM" || Designation == "RSM")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where REGION_CODE='" + LocCodeSession + "' AND DESIGNATION='" + Designation + "' ";

                }
                if (Designation == "MPO")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where REGION_CODE='" + LocCodeSession + "' AND DESIGNATION IN ('MPO','SMPO') ";
            ...
            if (EmpTypeSession == "DSM" || EmpTypeSession == "Executive" || EmpTypeSession == "Sr. Executive")
            {
                if (Designation == "TM" || Designation == "RSM")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where DIVISION_CODE='" + LocCodeSession + "' AND DESIGNATION='" + Designation + "' ";
                }
                if (Designation == "DSM")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where DIVISION_CODE='" + LocCodeSession + "' AND DESIGNATION IN ('DSM','Manager','Sr. Manager') ";
                }
                if (Designation == "MPO")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where DIVISION_CODE='" + LocCodeSession + "'  AND DESIGNATION IN ('MPO','SMPO')";
                }
            }
            if (EmpTypeSession == "Manager" || EmpTypeSession == "Sr. Manager")
            {
                if (Designation == "TM" || Designation == "RSM" )
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where M_ID='" + LocCodeSession + "' AND DESIGNATION='" + Designation + "' ";
                }
                if (Designation == "DSM")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||', '||DESIGNATION||' |'||MARKET_NAME EmpName from VW_HR_LOC_MAPPING_ALL Where M_ID='" + LocCodeSession + "' AND DESIGNATION IN ('DSM','Manager','Sr. Manager') ";
                }
                if (Designation == "MPO")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where M_ID='" + LocCodeSession + "'  AND DESIGNATION IN ('MPO','SMPO')";
                }
            }
            if (EmpTypeSession == "EMA")
            {
                if (Designation == "TM" || Designation == "RSM")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where   DESIGNATION='" + Designation + "' ";
                }
                if (Designation == "DSM")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where   DESIGNATION IN ('DSM','Manager','Sr. Manager') ";
                }
                if (Designation == "MPO")
                {
                    Qry = " Select MPO_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName from VW_HR_LOC_MAPPING_ALL Where   DESIGNATION IN ('MPO','SMPO')";
                }
            }            
            DataTable dt = dbHelper.GetDataTable(dbConn.SAConnStrReader(), Qry);
            List<DefaultBEL> item;
        ...
        public List<DefaultBEL> GetEmpForDesignationLocCodeSession(string RegionCode, string Designation)
        {
           ...
           // string WhereClause = GetImmediateSubordinate_WhereClauseDefineAsUserLocCode(LocCodeSession, EmpTypeSession);
            ...
            if (RegionCode != null && RegionCode != "" && RegionCode != " " && RegionCode != "null" && Designation != "DSM")
            {
                Qry = Qry + " AND REGION_CODE ='" + RegionCode + "'";
            }
            /*   
        ...
        public List<DefaultBEL> GetEmpForDesignation(string Designation,string OperationMode)
        {
            ...
            if (Designation == "DSM")
            {
                Qry = Qry + " Where DESIGNATION IN ('DSM','Manager','Sr. Manager') ";
            }
            if (Designation == "MPO")
             {
        ...
        {
            string Qry = "Select  LOC_CODE,MPO_NAME||' - '||MPO_CODE||', '||DESIGNATION||' | '||MARKET_NAME||' - '||MARKET_CODE EmpName From VW_HR_LOC_MAPPING_ALL Where DESIGNATION='TM'  ";
        ...
        public List<DefaultBEL> GetEmpForOwnSupfrmDepotDesignation(string DepotCode, string Designation, string SBU)
        {
            ...
            if (EmpTypeSession == "EMA")
            {
                ...
                else
                {
                    ...
                    if (Designation == "RSM")
                    {
                        Qry = "Select distinct REGION_CODE LOC_CODE,REGION_NAME LocName  from HR_LOC_MAPPING Where DESIGNATION='RSM'";
                    }
                    if (Designation == "TM")
                    {
                        Qry = "Select distinct TERRITORY_CODE LOC_CODE,TERRITORY_NAME LocName from HR_LOC_MAPPING Where DESIGNATION='TM'";
                    }
                    if (Designation == "MPO")
                    {
                        Qry = "Select distinct MARKET_CODE || MARKET_GROUP || PRODUCT_GROUP LOC_CODE,MARKET_NAME LocName  from HR_LOC_MAPPING Where DESIGNATION IN ('MPO','SMPO')";
                    }
        ...
        public List<DefaultBEL> GetEmployeeForArchive(DefaultBEL model)
        {
            ...
            if (model.Designation == "TM" || model.Designation == "RSM" )
            {
